...
void Cluster_set_option::prepare() {
  ...
  // Replicaset_set_option object to handle it. If the option it not a
  ...
  if (k_global_replicaset_supported_options.count(m_option) != 0) {
    ...
    if (!m_value_str.is_null()) {
      m_replicaset_set_option = shcore::make_unique<Replicaset_set_option>(
          default_rs, m_option, *m_value_str);
    } else {
      m_replicaset_set_option = shcore::make_unique<Replicaset_set_option>(
          default_rs, m_option, *m_value_int);
    }
