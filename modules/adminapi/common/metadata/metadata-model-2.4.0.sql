...
/*
  ...
  * 2.2.0 - Added support for Read-Replicas
  * 2.3.0 - Added support for Routing Guidelines
  * 2.4.0 - Added new table to store Router statistics (router_stats)
...
CREATE SQL SECURITY INVOKER VIEW schema_version (major, minor, patch) AS SELECT 2, 4, 0;
...
CREATE TABLE IF NOT EXISTS instances (
  ...
  */
  ...
  /*
    Whether the instance belongs to Cluster, a ReplicaSet, or is a Read-Replica.
   */
  `instance_type` ENUM('group-member', 'async-member', 'read-replica') NOT NULL,
...
CREATE TABLE IF NOT EXISTS routers (
  ...
  */
  ...
  /*
    ...
    Managed by Router. (Deprecated in favor of 'router_stats.last_check_in')
   */
...
/*
  This table stores per-router runtime statistics that change frequently. The `routers` table holds larger JSON configuration that changes rarely; separating stats here avoids rewriting large rows and allows lightweight, frequent updates.

  One row per router (1:1 with routers.router_id).
*/
...
CREATE TABLE IF NOT EXISTS routing_guidelines (
  /*
    Unique ID of a Routing Guideline record.
  */
  `guideline_id` VARCHAR(36) CHARACTER SET ascii COLLATE ascii_general_ci PRIMARY KEY,
  /*
    Unique name for the Routing Guideline.
   */
  `name` VARCHAR(63) NOT NULL,
  /*
    Contains the Routing Guideline JSON document
  */
  `guideline` JSON,
  /*
    Id of the ClusterSet this Routing Guideline belongs to, if any.
  */
  `clusterset_id` VARCHAR(36) CHARACTER SET ascii COLLATE ascii_general_ci,
  /*
    ID of the Cluster/ReplicaSet this Routing Guideline belongs to
  */
  `cluster_id` VARCHAR(36) CHARACTER SET ascii COLLATE ascii_general_ci,
  /*
    A timestamp updated every time the Routing Guideline is changed
  */
  `last_update` TIMESTAMP,
  /*
    TRUE if this Routing Guideline is the default
  */
  `default_guideline` BOOL NOT NULL,
  FOREIGN KEY (clusterset_id) REFERENCES clustersets (clusterset_id),
  FOREIGN KEY (cluster_id) REFERENCES clusters (cluster_id),
  UNIQUE KEY(name)
) CHARSET = utf8mb4, ROW_FORMAT = DYNAMIC;
...
CREATE SQL SECURITY INVOKER VIEW v2_instances AS
  SELECT i.instance_id,
         ...
         JSON_INSERT(i.attributes, '$.instance_type', i.instance_type) as attributes,
         i.instance_type
  FROM instances i;
...
DROP PROCEDURE IF EXISTS v2_set_global_cs_router_option//
CREATE PROCEDURE v2_set_global_cs_router_option(
  id VARCHAR(36),
...
DROP PROCEDURE IF EXISTS v2_set_global_cluster_router_option//
CREATE PROCEDURE v2_set_global_cluster_router_option(
  id VARCHAR(36),
  option_name VARCHAR(100),
  option_value JSON)
SQL SECURITY INVOKER
...
DROP PROCEDURE IF EXISTS v2_set_cs_routing_option//
CREATE PROCEDURE v2_set_cs_routing_option(
  router VARCHAR(512),
...
DROP PROCEDURE IF EXISTS v2_set_cluster_routing_option//
CREATE PROCEDURE v2_set_cluster_routing_option(
  router VARCHAR(512),
  cluster_id VARCHAR(36),
  option_name VARCHAR(100),
  option_value JSON)
...
BEGIN
  ...
  if 0 = (select count(*) from mysql_innodb_cluster_metadata.routers r
          where r.cluster_id = cluster_id and
          r.address = router_address and r.router_name = router_name) then
    SET errmsg = concat('Router ', QUOTE(router), ' is not part of this Cluster');
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = errmsg;
  end if;
  ...
  if option_value IS NULL then
      UPDATE mysql_innodb_cluster_metadata.routers r
        SET r.options = JSON_REMOVE(r.options, concat('$.', option_name))
        WHERE r.cluster_id = cluster_id AND r.address = router_address AND r.router_name = router_name;
  else
      UPDATE mysql_innodb_cluster_metadata.routers r
        SET r.options = JSON_SET(IFNULL(r.options, '{}'), concat('$.', option_name), option_value)
        WHERE r.cluster_id = cluster_id AND r.address = router_address AND r.router_name = router_name;
...
/*
  List of router options per router.
  Merges global values if router specific values are not set.
  Precedence order: Router > Cluster > ClusterSet
 */
...
CREATE SQL SECURITY INVOKER VIEW v2_router_options AS
  SELECT
        ...
        r.cluster_id,
        r.clusterset_id,
        JSON_MERGE_PATCH((SELECT COALESCE((SELECT router_options
                          FROM mysql_innodb_cluster_metadata.clustersets
                          WHERE clusterset_id = r.clusterset_id), '{}')),
                          (SELECT COALESCE((SELECT router_options
