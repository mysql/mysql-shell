...
struct Init_uuid_gen {
  Init_uuid_gen() {
    ...
    boost::mt19937 gen;
    boost::variate_generator<boost::mt19937 &, boost::uniform_int<> > vargen(
        gen, dist);
    gen.seed(now);
    init_uuid(vargen());
  }
...
REGISTER_HELP(
    ...
    "@li Passing several parameters to the function, each parameter should be a document definition.");
...
REGISTER_HELP(
    ...
    "To be added, every document must have a string property named '_id' ideally with a universal unique identifier (UUID) as value. "
...
shcore::Value CollectionAdd::add(const shcore::Argument_list &args) {
  ...
  if (raw_owner) {
    std::shared_ptr<Collection> collection(
        std::static_pointer_cast<Collection>(raw_owner));
      ...
      try {
        ...
        if (args.size() == 1) {
          if (args[0].type == Map ||
            ...
            // On a single document parameter, creates an array and processes it
            // as a list of
            // documents, only advantage of this is avoid duplicating validation
            // and setup logic
            ...
            throw shcore::Exception::argument_error(
                "Argument is expected to be either a document or a list of documents");
        } else {
        ...
        if (shell_docs) {
          if (!_add_statement.get())
            _add_statement.reset(
                new ::mysqlx::AddStatement(collection->_collection_impl));
          ...
          for (index = 0; index < size; index++) {
            ...
            if (element.type == Map)
              shell_doc = element.as_map();
                     ...
                     element.as_object()->class_name() == "Expression") {
              std::shared_ptr<mysqlx::Expression> expression =
                  std::static_pointer_cast<mysqlx::Expression>(
                      element.as_object());
              shcore::Value document =
                  shcore::Value::parse(expression->get_data());
            ...
            if (shell_doc) {
              ...
              // No matter how the document was received, gets passed as
              // expression to the
              // backend
              inner_doc.reset(shcore::Value(shell_doc).json(), true,
                              (*shell_doc)["_id"].as_string());
            }
...
shcore::Value CollectionAdd::execute(const shcore::Argument_list &args) {
  ...
  try {
    ...
    MySQL_timer timer;
    timer.start();
    result.reset(new mysqlx::Result(safe_exec(*_add_statement)));
    timer.end();
