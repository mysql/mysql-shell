...
class XShell_TestCases(unittest.TestCase):
    ...
    def test_004_MYS_735_createCluster_InvalidParams(self):
      '''MYS-735 [MYAA] CreateCluster() using invalid number of parameters'''
      ...
      for command, response in x_cmds:
          ...
          try:
              found = stdout.find(response, 0, len(stdout))
              if found == -1:
              ...
              if found == -1:
                  logger.debug(
                      "exec_xshell_command: handled exception return: " + "FAIL:  " + str(stdout) + str(stderr))
                  results = 'FAIL'
              else:
                  logger.debug("exec_xshell_command: return PASS")
                  results = 'PASS'
