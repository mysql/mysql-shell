...
class XShell_TestCases(unittest.TestCase):
  ...
  def test_015_MYS_757_startSandboxInstance(self):
      '''MYS-757 [MYAA] dba.startSandboxInstance(port[, options]) with sandboxDir parameter'''
      logger.debug("--------- " + str(self._testMethodName) + " ---------")
      instance="3313"
      time.sleep(5)
      results = ''
      init_command =  [MYSQL_SHELL, '--interactive=full', '--passwords-from-stdin']
      x_cmds = [("dba.startSandboxInstance("+instance+", { sandboxDir: \""+cluster_Path+"\"});\n",'successfully started.')]
