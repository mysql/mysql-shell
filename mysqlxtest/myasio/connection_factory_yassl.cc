...
: m_is_client(is_client)
{
  ...
  ssl_ctxt = boost::shared_ptr<yaSSL::SSL_CTX>(SSL_CTX_new(method), SSL_CTX_free);

  mysqld::set_context(ssl_ctxt.get(), m_is_client, ssl_key, ssl_cert, ssl_ca,
                      ssl_ca_path, ssl_cipher, ssl_crl, ssl_crl_path);
}

Connection_unique_ptr Connection_yassl_factory::create_connection(boost::asio::io_service &io_service)
{
  Wrapper_ssl_ptr       wrapper_ssl(new Wrapper_yassl(ssl_ctxt));
  Connection_unique_ptr connection(new Connection_raw<boost::asio::ip::tcp::socket>(io_service));
  ...
  Connection_unique_ptr connection_ssl(new Connection_yassl(boost::move(connection), boost::move(wrapper_ssl), handlers));

  return Connection_unique_ptr(new Connection_dynamic_tls(boost::move(connection_ssl)));
}

Options_context_ptr Connection_yassl_factory::create_ssl_context_options()
{
  return Options_context_ptr(new Options_context_ssl(ssl_ctxt.get()));
}
