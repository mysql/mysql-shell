...
Connection_yassl::Connection_yassl(Connection_unique_ptr connection_raw, Wrapper_ssl_ptr ssl, Vector_states_yassl &handlers)
: m_connection(boost::move(connection_raw)),
  m_ssl(boost::move(ssl)),
  m_callback(boost::make_shared<Callback_post>(boost::bind(&Connection::post, m_connection.get(), _1))),
  m_ssl_pdu_input_buffer(new Buffer_page()),
...
Options_session_ptr Connection_yassl::options()
{
  return m_ssl->get_ssl_options();
}

Connection_unique_ptr Connection_yassl::get_lowest_layer()
{
