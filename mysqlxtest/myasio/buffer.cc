...
Alloc_result Buffer::reserve(size_t space)
{
  ...
  while (available < space)
  {
    ...
    catch (const Page_pool::No_more_pages_exception &exc)
    {
...
std::vector<boost::asio::mutable_buffer> Buffer::get_asio_buffer()
{
  std::vector<boost::asio::mutable_buffer> b;
  b.reserve(pages().size());
  ...
  for (Page_list::const_iterator p = pages().begin(); p != pages().end(); ++p)
  {
    ...
    {
      b.push_back(boost::asio::buffer((*p)->data + (*p)->length, (*p)->capacity - (*p)->length));
    }
  }
