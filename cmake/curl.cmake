...
FUNCTION(CURL_CONFIG option output_var)
  SET(CURL_CONFIG_EXECUTABLE ${WITH_CURL}/bin/curl-config)
  EXECUTE_PROCESS(
    COMMAND ${CURL_CONFIG_EXECUTABLE} ${option}
    OUTPUT_VARIABLE output
    OUTPUT_STRIP_TRAILING_WHITESPACE
  )
  MESSAGE(STATUS "${CURL_CONFIG_EXECUTABLE} ${option}: ${output}")
  SET(${output_var} ${output} PARENT_SCOPE)
ENDFUNCTION()

MACRO(MYSQL_CHECK_CURL)
  IF(NOT WITH_CURL)
    MESSAGE(STATUS "No WITH_CURL has been set, defaulting to 'system'.")
  ...
  ELSEIF(WITH_CURL)
    ...
    IF(WIN32)
      # on Windows we're always linking statically
      SET(CURL_DEFINITIONS "CURL_STATICLIB")
    ELSE()
      CURL_CONFIG("--libs" curl_config_libs)

      STRING(REGEX REPLACE ".* -lcurl *(.*)" "\\1" curl_config_libs "${curl_config_libs}")
      STRING(REGEX REPLACE "-l\([^ ]+\)" "\\1" curl_config_libs "${curl_config_libs}")
      SEPARATE_ARGUMENTS(curl_config_libs)
      SET(CURL_LIBRARY ${CURL_LIBRARY} ${curl_config_libs})

      CURL_CONFIG("--built-shared" curl_config_shared)

      IF(curl_config_shared STREQUAL "no")
        SET(CURL_DEFINITIONS "CURL_STATICLIB")
      ELSE()
        SET(CURL_DEFINITIONS "")
      ENDIF()
  ...
  MESSAGE(STATUS "CURL_DEFINITIONS = ${CURL_DEFINITIONS}")
ENDMACRO()
