...
using base::MySQLVersion;

namespace parsers {

//---------------------------------------------------------------------------------------------------------------------

static void replaceStringInplace(std::string &value, const std::string &search,
                                 const std::string &replacement) {
  std::string::size_type next;

  for (next = value.find(search); next != std::string::npos;
...
bool MySQLRecognizerCommon::isSqlModeActive(size_t mode) const {
...
static void trim(std::string &s) {
  std::locale locale;
  s.erase(s.begin(), std::find_if_not(s.begin(), s.end(), [&locale](char c) {
...
void MySQLRecognizerCommon::sqlModeFromString(std::string modes) {
  ...
  while (std::getline(iss, mode, ',')) {
    ...
    else if (mode == "HIGH_NOT_PRECEDENCE" || mode == "MYSQL323" ||
             mode == "MYSQL40")
...
static std::string dumpTree(RuleContext *context, const Vocabulary &vocabulary,
  ...
  for (size_t index = 0; index < context->children.size(); ++index) {
    ...
    if (antlrcpp::is<RuleContext *>(child)) {
      ...
      if (antlrcpp::is<MySQLParser::TextLiteralContext *>(child)) {
        auto interval = ruleContext->getSourceInterval();
        stream << indentation << "(index range: " << interval.a << ".."
               << interval.b << ", string literal) "
      ...
      stream << "(line: " << token->getLine()
             << ", offset: " << token->getCharPositionInLine()
             << ", index: " << token->getTokenIndex() << ", " << tokenName
...
std::string MySQLRecognizerCommon::dumpTree(
    RuleContext *context, const antlr4::dfa::Vocabulary &vocabulary) {
  return ::parsers::dumpTree(context, vocabulary, "");
}

//----------------------------------------------------------------------------------------------------------------------

std::string MySQLRecognizerCommon::sourceTextForContext(ParserRuleContext *ctx,
                                                        bool keepQuotes) {
...
std::string MySQLRecognizerCommon::sourceTextForRange(ParseTree *start,
                                                      ParseTree *stop,
                                                      bool keepQuotes) {
  Token *startToken = antlrcpp::is<TerminalNode *>(start)
                          ? dynamic_cast<TerminalNode *>(start)->getSymbol()
                          : dynamic_cast<ParserRuleContext *>(start)->start;
  Token *stopToken = antlrcpp::is<TerminalNode *>(stop)
                         ? dynamic_cast<TerminalNode *>(start)->getSymbol()
...
std::string MySQLRecognizerCommon::sourceTextForRange(Token *start, Token *stop,
                                                      bool keepQuotes) {
  CharStream *cs = start->getTokenSource()->getInputStream();
  size_t stopIndex = stop != nullptr ? stop->getStopIndex()
                                     : std::numeric_limits<size_t>::max();
  std::string result = cs->getText(Interval(start->getStartIndex(), stopIndex));
  if (keepQuotes || result.size() < 2) return result;

  char quoteChar = result[0];
  if ((quoteChar == '"' || quoteChar == '`' || quoteChar == '\'') &&
      quoteChar == result.back()) {
    if (quoteChar == '"' || quoteChar == '\'') {
      // Replace any double occurence of the quote char by a single one.
      replaceStringInplace(result, std::string(2, quoteChar),
                           std::string(1, quoteChar));
    }
...
ParseTree *getPreviousSibling(ParseTree *tree) {
  ...
  if (parent->children.front() == tree) return nullptr;
...
ParseTree *MySQLRecognizerCommon::getPrevious(ParseTree *tree) {
  do {
    ...
    if (sibling != nullptr) {
      if (antlrcpp::is<TerminalNode *>(sibling)) return sibling;

      tree = sibling;
      while (!tree->children.empty()) tree = tree->children.back();
      if (antlrcpp::is<TerminalNode *>(tree)) return tree;
    } else {
      tree = tree->parent;
    }
  } while (tree != nullptr);
...
ParseTree *getNextSibling(ParseTree *tree) {
  ...
  if (parent->children.back() == tree) return nullptr;
...
/**
 * Returns the terminal node at the given position. If there is no terminal node
 * at the given position (or if it is EOF) then the previous terminal node is
 * returned instead. If there is none then the one after the position is
 * returned instead (which could also be EOF). Note: the line is one-based.
 */
ParseTree *MySQLRecognizerCommon::terminalFromPosition(
    ParseTree *root, std::pair<size_t, size_t> position) {
  do {
    root = getNext(root);
    if (antlrcpp::is<TerminalNode *>(root)) {
      Token *token = static_cast<TerminalNode *>(root)->getSymbol();
      if (token->getType() == Token::EOF) return getPrevious(root);

      // If we reached a position after the given one then we found a situation
      // where that position is between two terminals. Return the previous one
      // in this case.
      if (position.second < token->getLine()) return getPrevious(root);
      if (position.second == token->getLine() &&
          position.first < token->getCharPositionInLine())
        return getPrevious(root);

      size_t length = token->getStopIndex() - token->getStartIndex() + 1;
      if (position.second == token->getLine() &&
          (position.first < token->getCharPositionInLine() + length))
        return root;
    }
  } while (root != nullptr);
...
static bool treeContainsPosition(ParseTree *node, size_t position) {
  ...
  if (terminal != nullptr) {
    return terminal->getSymbol()->getStartIndex() <= position &&
           position <= terminal->getSymbol()->getStopIndex();
  }

  auto context = dynamic_cast<ParserRuleContext *>(node);
  if (context == nullptr) return false;

  return context->start->getStartIndex() <= position &&
         position <= context->stop->getStopIndex();
}

//----------------------------------------------------------------------------------------------------------------------

/**
 * Returns the parser context at the given character index position or nullptr
 * if there's none.
 */
ParseTree *MySQLRecognizerCommon::contextFromPosition(ParseTree *root,
                                                      size_t position) {
  if (!treeContainsPosition(root, position)) return nullptr;
  ...
  // No child contains the given position, so it must be in whitespaces between
  // them. Return the root for that case.
  return root;
}

//----------------------------------------------------------------------------------------------------------------------

SymbolTable *functionSymbolsForVersion(MySQLVersion version) {
  static std::map<MySQLVersion, SymbolTable> functionSymbols;

  if (functionSymbols.count(version) == 0) {
    const auto &functions = MySQLSymbolInfo::systemFunctionsForVersion(version);
    SymbolTable &symbolTable =
        functionSymbols[version];  // Creates the new symbol table.

    for (const auto &function : functions) {
      symbolTable.addNewSymbol<SystemFunctionSymbol>(nullptr, function,
                                                     nullptr);
    }
  }
