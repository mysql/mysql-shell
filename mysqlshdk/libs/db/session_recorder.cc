...
std::unique_ptr<IResult> Session_recorder::query(const std::string& sql, bool buffered) {
  // While mock recording, all is buffered
  auto result = _target->query(sql, true);

  std::unique_ptr<IResult> ret_val(new Result_recorder(result.get()));
...
const char* Session_recorder::get_ssl_cipher() {
  ...
  Mock_record::get() << "EXPECT_CALL(session, get_ssl_cipher().WillOnce(Return(\"" << cipher << "\"));" << std::endl;
  return cipher;
}
...
void Result_recorder::save_current_result(const std::vector<Column>& columns, const std::vector<std::unique_ptr<IRow> >& rows) {
...
int64_t Result_recorder::get_auto_increment_value() const {
  ...
  Mock_record::get() << "EXPECT_CALL(result, get_auto_increment_value().WillOnce(Return(" << ret_val << "));" << std::endl;
  return ret_val;
}

bool Result_recorder::has_resultset() {
  auto ret_val = _target->has_resultset();
  Mock_record::get() << "EXPECT_CALL(result, has_resultset().WillOnce(Return(" << (ret_val ? "true" : "false") << "));" << std::endl;
  return ret_val;
}

uint64_t Result_recorder::get_affected_row_count() const {
  auto ret_val = _target->get_affected_row_count();
  Mock_record::get() << "EXPECT_CALL(result, get_affected_row_count().WillOnce(Return(" << ret_val << "));" << std::endl;
  return ret_val;
}
uint64_t Result_recorder::get_fetched_row_count() const {
  auto ret_val = _target->get_fetched_row_count();
  Mock_record::get() << "EXPECT_CALL(result, get_fetched_row_count().WillOnce(Return(" << ret_val << "));" << std::endl;
  return ret_val;
}

uint64_t Result_recorder::get_warning_count() const {
  auto ret_val = _target->get_warning_count();
  Mock_record::get() << "EXPECT_CALL(result, get_warning_count().WillOnce(Return(" << ret_val << "));" << std::endl;
  return ret_val;
}

unsigned long Result_recorder::get_execution_time() const {
  auto ret_val = _target->get_execution_time();
  Mock_record::get() << "EXPECT_CALL(result, get_execution_time().WillOnce(Return(" << ret_val << "));" << std::endl;
  return ret_val;
}

std::string Result_recorder::get_info() const {
  auto ret_val = _target->get_info();
  Mock_record::get() << "EXPECT_CALL(result, get_info().WillOnce(Return(\"" << ret_val << "\"));" << std::endl;
  return ret_val;
}
