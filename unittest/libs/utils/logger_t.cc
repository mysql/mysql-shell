// TODO(alfredo) this needs to be updated to be integrated into the rest of the
// tests
...
class LoggerTestProxy  // this class is a friend of Logger, thus allows testing
                       // of its private methods
{
 public:
  static std::string format_message(const char *domain, const char *message,
...
TEST(Logger, simple) {
  ...
  size_t idx =
      contents->find("Error: Unit Test Domain: Error due to critical\n");
  size_t idx2 = contents->find(
...
#ifdef _WIN32
    EXPECT_EQ(std::string("Error in Logger::Logger when opening file '") +
                  PROCESS_LAUNCHER_TESTS_DIR + "/' for writing",
              e.what());
#else
    EXPECT_EQ(
        std::string(
            "Error in Logger::Logger when opening file '/tmp/' for writing"),
        e.what());
#endif
  } catch (...) {
    FAIL() << "Expected std::logic_error";
...
TEST(Logger, format_message) {
  ...
  // example expected message: "2015-12-23 09:26:49: Debug: Unit Test Domain:
  // Some message"
  {
    std::string s = LoggerTestProxy::format_message(
        "Unit Test Domain", "Some message", Logger::LOG_DEBUG);
    EXPECT_STREQ(": Debug: Unit Test Domain: Some message",
                 s.c_str() + 19);  // +19 to skip the timestamp
    EXPECT_TRUE(is_timestamp(s.c_str()));
  }

  // example expected message: "2015-12-23 09:26:49: Info: Unit Test Domain:
  // Some message"
  {
    std::string s = LoggerTestProxy::format_message(
        "Unit Test Domain", "Some message", Logger::LOG_INFO);
    EXPECT_STREQ(": Info: Unit Test Domain: Some message",
                 s.c_str() + 19);  // +19 to skip the timestamp
    EXPECT_TRUE(is_timestamp(s.c_str()));
  }

  // example expected message: "2015-12-23 09:26:49: Warning: Unit Test Domain:
  // Some message"
  {
    std::string s = LoggerTestProxy::format_message(
        "Unit Test Domain", "Some message", Logger::LOG_WARNING);
    EXPECT_STREQ(": Warning: Unit Test Domain: Some message",
                 s.c_str() + 19);  // +19 to skip the timestamp
    EXPECT_TRUE(is_timestamp(s.c_str()));
  }

  // example expected message: "2015-12-23 09:26:49: Error: Unit Test Domain:
  // Some message"
  {
    std::string s = LoggerTestProxy::format_message(
        "Unit Test Domain", "Some message", Logger::LOG_ERROR);
    EXPECT_STREQ(": Error: Unit Test Domain: Some message",
                 s.c_str() + 19);  // +19 to skip the timestamp
    EXPECT_TRUE(is_timestamp(s.c_str()));
  }
  ...
  // Some exception"
  {
    ...
    std::string s = LoggerTestProxy::format_message(
        "Unit Test Domain", "Some message that will get ignored", e);
    EXPECT_STREQ(": Error: Unit Test Domain: Some exception",
                 s.c_str() + 19);  // +19 to skip the timestamp
    EXPECT_TRUE(is_timestamp(s.c_str()));
  }
}
