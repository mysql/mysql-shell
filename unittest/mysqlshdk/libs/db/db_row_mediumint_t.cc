...
TEST_F(Db_tests, row_getters_mediumint) {
  do {
    ...
    ASSERT_NO_THROW(session->connect(Connection_options(uri())));
    {
      ...
      CHECK_EQ(1, 8388607, get_int);
      NEXT_ROW();
      CHECK_EQ(0, 1, get_int);
      CHECK_EQ(1, 16777214, get_int);
      NEXT_ROW();
      CHECK_EQ(0, 8388607, get_int);
      CHECK_EQ(1, 16777215, get_int);
      NEXT_ROW();
      CHECK_NULL(0);
      CHECK_NULL(1);
      LAST_ROW();
