...
namespace shcore {
  class Shell_py_mysqlx_collection_add_tests : public Crud_test_wrapper
  {
  protected:
  ...
  TEST_F(Shell_py_mysqlx_collection_add_tests, initialization)
  {
  ...
  TEST_F(Shell_py_mysqlx_collection_add_tests, chain_combinations)
  {
    ...
    exec_and_out_equals("crud = collection.add([])");
  ...
  TEST_F(Shell_py_mysqlx_collection_add_tests, add_validations)
  {
    exec_and_out_equals("import mysqlx");
    exec_and_out_equals("session = mysqlx.getSession('" + _uri + "')");
    exec_and_out_equals("collection = session.py_shell_test.getCollection('collection1')");
  ...
  TEST_F(Shell_py_mysqlx_collection_add_tests, add_execution)
  {
    exec_and_out_equals("import mysqlx");
    exec_and_out_equals("session = mysqlx.getSession('" + _uri + "')");
    exec_and_out_equals("collection = session.py_shell_test.getCollection('collection1')");
