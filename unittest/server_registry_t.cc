...
namespace server_registry_tests
{
  TEST(Server_registry_test, merge)
  {
    ...
    Connection_options cs = sr->add_connection_options("8ab125f8-9678-d011-9c41-f816549b70d4", "protocol=mysql; app=dev connection; server=localhost; port=3305; user=root; password=123; custom1=my custom value;;");
  ...
  {
    std::string file = shcore::get_user_config_path();
    ...
    for (std::map<std::string, Connection_options>::const_iterator it = sr2->begin(); it != sr2->end(); ++it)
    {
      ...
      if (csi.get_uuid() == "8ab125f8-9678-d011-9c41-f816549b70d4")
      {
        ...
        EXPECT_STREQ(csi.get_name().c_str(), "dev connection");
