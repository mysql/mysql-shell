...
Invalid options in create collection options dictionary: vals (ArgumentError)

//@<ERR> create collection with wrong validation type
Option 'validation' is expected to be of type Map, but is String (TypeError)

//@<ERR> create collection with malformed validation object
'whatever' is not a valid option (MYSQLSH 5021)

//@<ERR> create collection with incorrect validation schema {VER(>= 8.0.19)}
The provided validation schema is invalid (JSON validation schema location #/name failed requirement: 'additionalProperties' at meta schema location '#'). (MYSQLSH 5182)

//@<ERR> create collection unsupported server
The target MySQL server does not support options in the createCollection function (MYSQLSH 5015)
The target MySQL server does not support options in the createCollection function (MYSQLSH 5015)
...
Missing values in modify collection options: validation (ArgumentError)
...
The target MySQL server does not support the requested operation. (MYSQLSH 5157)

//@<ERR> modify collection validation block fails because of data inconsistencies {VER(>= 8.0.19)}
Document is not valid according to the schema assigned to collection. The JSON document location '#/i' failed requirement 'type' at JSON Schema location '#/properties/i'. (MYSQLSH 5180)
