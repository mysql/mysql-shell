...
SSH_URI_DATA = shell.parse_uri(SSH_URI)
...
SSH_USER_URI_DATA = shell.parse_uri(SSH_USER_URI)
...
MYSQL_OVER_SSH_URI_DATA = shell.parse_uri(MYSQL_OVER_SSH_URI)
...
def create_ssh_dir():
  tmpsshdir = os.path.join(os.environ["TMPDIR"], "tmpssh")
  if os.path.exists(tmpsshdir):
      shutil.rmtree(tmpsshdir)
  os.mkdir(tmpsshdir)
  os.chmod(tmpsshdir, 0o700)
  return tmpsshdir


def create_ssh_conf(path, config, fix_permissions=True):
    testutil.create_file(path, config)
    if fix_permissions:
        import stat
        os.chmod(path, 0o600)
