...
EXPECT_STDOUT_CONTAINS("2) Usage of db objects with names conflicting with new reserved keywords")
EXPECT_STDOUT_CONTAINS("(reservedKeywords)")
EXPECT_STDOUT_CONTAINS("3) Usage of utf8mb3 charset (utf8mb3)")
EXPECT_STDOUT_CONTAINS("4) Table names in the mysql schema conflicting with new tables in the latest")
EXPECT_STDOUT_CONTAINS("MySQL. (mysqlSchema)")
EXPECT_STDOUT_CONTAINS("5) Partitioned tables using engines with non native partitioning")
EXPECT_STDOUT_CONTAINS("(nonNativePartitioning)")
EXPECT_STDOUT_CONTAINS("6) Foreign key constraint names longer than 64 characters (foreignKeyLength)")
EXPECT_STDOUT_CONTAINS("7) Usage of obsolete MAXDB sql_mode flag (maxdbSqlModeFlags)")
EXPECT_STDOUT_CONTAINS("8) Usage of obsolete sql_mode flags (obsoleteSqlModeFlags)")
EXPECT_STDOUT_CONTAINS("9) ENUM/SET column definitions containing elements longer than 255 characters")
EXPECT_STDOUT_CONTAINS("(enumSetElementLength)")
EXPECT_STDOUT_CONTAINS("27) MySQL syntax check for routine-like objects (syntax)")
WIPE_OUTPUT()
