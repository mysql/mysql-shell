...
# WL13804: WL13807-FR4.13.4 - If the `bytesPerChunk` option is not given and the `chunking` option is set to `true`, a default value of `32M` must be used instead.
...
EXPECT_FAIL("RuntimeError", "Unable to acquire global read lock: MySQL Error 1227 (42000): Access denied; you need (at least one of) the RELOAD privilege(s) for this operation", types_schema, types_schema_tables, test_output_absolute, { "showProgress": False }, True)
...
EXPECT_FAIL("ArgumentError", "The osBucketName option is missing.", types_schema, types_schema_tables, 'oci+os://sakila')
...
EXPECT_STDOUT_CONTAINS("WARNING: Could not select a column to be used as an index for table `{0}`.`{1}`. Chunking has been disabled for this table, data will be dumped to a single file.".format(tested_schema, tested_table))
