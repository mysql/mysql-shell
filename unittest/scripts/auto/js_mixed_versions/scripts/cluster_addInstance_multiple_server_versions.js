...
}, "Cluster.addInstance: Instance does not support the following localAddress values of the cluster: '[::1]:"+ __mysql_sandbox_gr_port1 + ", [::1]:" + __mysql_sandbox_gr_port2 + "'. IPv6 addresses/hostnames are only supported by Group Replication from MySQL version >= 8.0.14 and the target instance version is 8.0.13.");
...
EXPECT_THROWS(function () {
    cluster.addInstance(__sandbox_uri2);
}, "Cluster.addInstance: Cannot use value '[::1]:"+ __mysql_sandbox_gr_port2 + "' for option localAddress because it has an IPv6 address which is only supported by Group Replication from MySQL version >= 8.0.14 but the cluster contains at least one member with version");
