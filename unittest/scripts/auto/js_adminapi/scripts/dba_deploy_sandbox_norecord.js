...
EXPECT_NO_THROWS(function() { dba.deploySandboxInstance(__mysql_sandbox_port1, {sandboxDir: __sandbox_path});});
...
EXPECT_THROWS(function() { dba.deploySandboxInstance(__mysql_sandbox_port1,{sandboxDir: __sandbox_path_invalid});}, `The sandbox dir path '${__sandbox_path_invalid}' is not valid: it does not exist.`);

//@<> BUG#28624006: Deploy sandbox with file instead of directory
var __file_path = __sandbox_path + "/file";
testutil.touch(__file_path);

EXPECT_THROWS(function() { dba.deploySandboxInstance(__mysql_sandbox_port1, {sandboxDir: __file_path});}, `The sandbox dir path '${__file_path}' is not valid: it is not a directory.`);
...
EXPECT_THROWS_TYPE(function () {
    dba.deploySandboxInstance(__mysql_sandbox_port1, {allowRootFrom: null, sandboxDir: __sandbox_path});
...
dba.deploySandboxInstance(__mysql_sandbox_port1, {allowRootFrom: '', password: 'root', sandboxDir: __sandbox_path});
...
dba.deploySandboxInstance(__mysql_sandbox_port1, {allowRootFrom: 'qqqq', password: 'root', sandboxDir: __sandbox_path});
shell.connect(__sandbox_uri1);
var result = session.runSql(select_root_accounts).fetchAll();
EXPECT_EQ(repr(["localhost", "root"]), repr(result[0]));
