...
// version >= 8.0.5, it should leave it ready for InnoDB Cluster, regardless of whether the instance is local or not.

// FR1_1 - Configure [REMOTE|LOCAL] instance not valid for InnoDB cluster using dba.configureInstance() with VALID server >= [8.0.5]
// connect to [REMOTE|LOCAL] Instance and configure using dba.configureInstance() using server version >=[8.0.5]
...
//@ FR_1 Configure instance not valid for InnoDB cluster {VER(>=8.0.5)}
// Also covers: ET_4
...
// be remotely persisted (for example: log_bin) and VALID servers >= [8.0.5]
...
// FR1.1_2 - Try to configure [LOCAL] instance using dba.configureInstance() with variable persisted and VALID servers >= [8.0.5]
...
// FR2_1 - Try to configure [REMOTE] instance using dba.configureInstance() and VALID servers >= [8.0.5]
...
// FR2_2 - Try to configure [LOCAL] instance using dba.configureInstance() and VALID servers >=[8.0.5]
// with server having 'persisted-globals-load' set to 'OFF'
...
//@ FR2_2 - Configure local instance using dba.configureInstance() with 'persisted-globals-load' set to 'OFF' with cnf file path {VER(>=8.0.5)}
...
// FR3 - When dba.configureInstance() is called on a MySQL instance with version < 8.0.5 or with
// 'persisted-globals-load' set to 'OFF', it should leave it ready for InnoDB Cluster
// NOTE: 'persisted-globals-load' does not exists in versions < 8.0.5 so we cannot set it
...
//@FR3.1_1 - Configure local instance with 'persisted-globals-load' set to 'OFF' {VER(<8.0.5)}
var sandbox_cnf1 = testutil.getSandboxConfPath(__mysql_sandbox_port1);
...
//@ FR5 Configure instance not valid for InnoDB cluster, with interaction enabled {VER(>=8.0.5)}
...
//              FR6_1 - Configure LOCAL instance using dba.configureInstance() with the NEW optional parameter 'interactive' on VALID servers >= [8.0.5]
...
//@ ET_10 - Call dba.configuereInstance() with interactive flag set to true and using clusterAdmin {VER(>=8.0.5)}
...
//@ ET_12 - Call dba.configuereInstance() with interactive flag set to true, clusterAdmin option and super_read_only=1 {VER(>=8.0.5)}
testutil.expectPassword("Password for new account: ", "newPwd");
testutil.expectPassword("Confirm password: ", "newPwd");
testutil.expectPrompt("Do you want to perform the required configuration changes?", "Y");
testutil.expectPrompt("Do you want to restart the instance after configuring it?", "y");
...
//@ ET_12_alt - Super read-only enabled and 'clearReadOnly' is set {VER(>=8.0.5)}
shell.connect(__sandbox_uri1);
set_sysvar(session, "super_read_only", 1);
EXPECT_EQ('ON', get_sysvar(session, "super_read_only"));
testutil.expectPassword("Password for new account: ", "newPwd");
...
//@ ET_12_alt - Super read-only enabled and 'clearReadOnly' is set 5.7 {VER(<8.0.5)}
shell.connect(__sandbox_uri1);
set_sysvar(session, "super_read_only", 1);
EXPECT_EQ('ON', get_sysvar(session, "super_read_only"));
testutil.expectPassword("Password for new account: ", "newPwd");
