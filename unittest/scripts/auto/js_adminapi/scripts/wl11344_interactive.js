...
//@ FR1-TS-01 Check persisted variables after create cluster {VER(>=8.0.5)}
...
//@ FR1-TS-01 reboot cluster and check persisted variables {VER(>=8.0.5)}
...
//@ FR1-TS-7 show persisted cluster variables {VER(>=8.0.5)}
print_persisted_variables(session);
...
//@ FR1-TS-7 reboot cluster and check persisted variables {VER(>=8.0.5)}
...
//@ FR2-TS-1 check persisted variables on instance 1 {VER(>=8.0.5)}
...
//@ FR2-TS-1 cluster status showing instance 2 is missing {VER(>=8.0.5)}
cluster.status();
...
//@ FR2-TS-1 cluster status showing instance 2 is back online {VER(>=8.0.5)}
cluster.status();
...
//@ FR2-TS-3 check that warning is displayed when adding instance with persisted-globals-load=OFF {VER(>=8.0.5)}
WIPE_SHELL_LOG();
...
//@ FR2-TS-4 Check that persisted variables match the ones passed on the arguments to create cluster and addInstance {VER(>=8.0.5)}
...
//@ FR2-TS-6 Warning is displayed on addInstance {VER(<8.0.5)}
...
//@ FR2-TS-8 Check that correct values were persisted and that instance rejoins automatically {VER(>=8.0.5)}
print_persisted_variables(s1);
print_persisted_variables(s2);
testutil.stopSandbox(__mysql_sandbox_port2);
testutil.waitMemberState(__mysql_sandbox_port2, "(MISSING)");
...
//@ FR2-TS-9 Check that correct values were persisted on instance 2 {VER(>=8.0.5)}
print_persisted_variables(s2);
...
//@ FR2-TS-9 Check that correct values are persisted and updated when instances are added and that instances rejoin automatically {VER(>=8.0.5)}
...
//@ FR5-TS-1 Check that persisted variables are updated/reset after removeCluster operation {VER(>=8.0.5)}
print_persisted_variables(s2);
print_persisted_variables(s1);
...
//@ FR5-TS-4 Check that persisted variables are updated/reset after removeCluster operation {VER(>=8.0.5)}
print_persisted_variables(s3);
cluster.removeInstance(__sandbox_uri2);
print_persisted_variables(s3);
print_persisted_variables(s2);
...
//@ FR5-Extra Check that warning is shown when removeInstance is called {VER(<8.0.5)}
WIPE_SHELL_LOG();
cluster.removeInstance(__sandbox_uri2);
...
//@ Check if Cluster dissolve will reset persisted variables {VER(>=8.0.5)}
print_persisted_variables(session);
