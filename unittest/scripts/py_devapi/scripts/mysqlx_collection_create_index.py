...
result = collection.createIndex('_name').field('name', "TEXT(50)", True).execute()
...
result = collection.createIndex('_name').field('alias', "TEXT(50)", True).execute()
...
result = collection.createIndex('_name', mysqlx.IndexType.Unique).field('name', "TEXT(50)", True).execute()

#@ ERROR: Attempt to create unique index when records are missing the key field
result = collection.createIndex('_alias', mysqlx.IndexType.Unique).field('alias', "TEXT(50)", True).execute()
