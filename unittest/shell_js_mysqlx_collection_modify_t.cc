...
namespace shcore {
  ...
  TEST_F(Shell_js_mysqlx_collection_modify_tests, modify_execution)
  {
...

    {
      ...
      exec_and_out_equals("var result = collection.modify('name = \"brian\"').set('alias', 'bri').set('last_name', 'black').set('age', mysqlx.expr('13+1')).execute();");
...

    {
      ...
      exec_and_out_equals("var result = collection.modify('name = :who').arrayInsert('girlfriends[1]','samantha').bind('who', 'brian').execute();");
      exec_and_out_equals("print(result.affectedRows)", "1");
