...
std::string Uri_encoder::encode_scheme(const std::string &data) {
  ...
  if (_tokenizer.tokens_available()) {
    ...
    if (_tokenizer.tokens_available())
      throw Uri_error((boost::format("Invalid scheme format [%1%], only one "
        "extension is supported") % data).str());
    else
      throw Uri_error((boost::format("Scheme extension [%1%] is not "
        "supported") % ext).str());
  }
  ...
  if (ret_val != "mysql" && ret_val != "mysqlx")
    throw Uri_error((boost::format("Invalid scheme [%1%], supported schemes "
      "include: mysql, mysqlx") % data).str());
...
std::string Uri_encoder::encode_port(const std::string& data) {
  ...
  if (port_token.get_type() != "digits")
    throw Uri_error((boost::format("Unexpected data [%1%] found in port "
      "definition") % port_token.get_text()).str());
...
std::string Uri_encoder::encode_values(const std::vector<std::string> &values,
  ...
  ret_val += shcore::join_strings(encoded_values, ",");
